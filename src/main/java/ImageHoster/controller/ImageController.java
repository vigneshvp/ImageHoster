...
public class ImageController {
    ...
    public String showImage(@PathVariable("imageId") int imageId,
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model,
        ...
        if (user.getId().equals(image.getUser().getId())) {
            //Go to edit screen if current user and user who created the image are same.
            return "images/edit";
        } else {
            //Stay on same page and pass error message in model.
    ...
    public String editImageSubmit(@RequestParam("file") MultipartFile file,
        ...
        Image updatedImage, HttpSession session, Model model) throws IOException {
        Image image = imageService.getImage(imageId);
        User user = (User) session.getAttribute("loggeduser");
        ...
        model.addAttribute("comments", updatedImage.getComments());
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId,
        ...
        if (imageToBeDeleted != null) {
            User user = (User) session.getAttribute("loggeduser");
            // Checks if current user and user who created the image are same.
            if (user.getId().equals(imageToBeDeleted.getUser().getId())) {
                imageService.deleteImage(imageId);
            } else {
                String error = "Only the owner of the image can delete the image";
                model.addAttribute("image", imageToBeDeleted);
                model.addAttribute("deleteError", error);
                model.addAttribute("comments", imageToBeDeleted.getComments());
                return "images/image";
            }
        }
        //Redirect to images if image is deleted
        return "redirect:/images";
    }
